...
| 12 | AI-POLICY → `docs/enterprise/` (table item #4) | **REVERSED → keep in `templates/`** *(owner-ratified 2026-06-24, during T3c B-group)* | recon revealed AI-POLICY is **one of 9 enterprise-governance fill-in templates** `scripts/incept.sh:221` stamps *as a set* (`templates/X-TEMPLATE.md` → `docs/governance/X.md`). Moving only it breaks set-coherence + needs an incept special-case or a capability drop, and mixes a fill-in template into `docs/enterprise/`'s reference docs. The "co-locate with the crosswalk" benefit is cosmetic (the crosswalk already references it by path). Same surface-pattern error the design-intent lens catches. **Group B collapses to just the frame/shape merge.** |

**Net corrected T3:** content-preserving consolidations (1,2,4,7) + relabel (9) + dead-build-doc retirement (8) + backlog reorg (E12/E14→E3, E13 dissolve, E8 defer) — and **keep** JIRA-SETUP (kept *with a tier-note*, not merged — owner-ratified during T3c), platform-safety-boundary, harness-enforcement-evidence, E9, E11, the 9 profiles' floating action pins (reversal #11), and **AI-POLICY in `templates/`** (reversal #12). This is right-weighting *by organization + honesty*, not subtraction of capability — fully consistent with the kit's front-load-rigor design.
...
- ~~**Move `AI-POLICY-TEMPLATE.md` → `docs/enterprise/`**~~ — **REVERSED (design-intent #12), kept in `templates/`**: it's one of the 9 governance templates `incept.sh:221` stamps as a set; moving only it breaks set-coherence for a cosmetic co-location gain.
...
- **Retire the 6 completed E-series design docs** (`2026-06-22-e4a-containment-audit`, `2026-06-23-{e4-work-mount-fix, e4a-prime-token-scope, e4b-image-vuln-gate, e4c-dast-runtime-security, e4e-author-not-approver-sod}`). Rationale is captured in CHANGELOG + live implementation. Keep `e3-…-design.md` (E3 not built yet — live spec).
  - **★ Execution correction (T3c PR2, v3.48.2):** the "atomic with the 5 companion plan docs or check-links breaks" rationale was **disproven against the mechanism** — `check-links.sh` scans only *tracked* `*.md` files and only validates relative Markdown link targets (the bracket-then-paren form); the plan docs are gitignored (untracked, never scanned/shipped) and the 6 design docs are referenced only by `ROADMAP-KIT.md` **backtick code-spans** (which carry no link target), not Markdown links. So PR2 = the 6 tracked design docs only + tidying the 6 stale ROADMAP "Design:" mentions; the plan docs are local scratch (no PR involvement).
