...
1. **`judges.py` — injection defense.** Refactor the prompt construction into a testable static `_build_prompt(prompt, candidate, expected, rubric)`:
   ...
   - `score()` calls `_build_prompt(...)`. The defense is unit-testable offline (no live call): assert the candidate is fenced, the fence appears exactly twice (open + close — breakout stripped to a fixed point), and an injection payload lands between the fences (the untrusted-data region).
