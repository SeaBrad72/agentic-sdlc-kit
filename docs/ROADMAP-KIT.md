...
- **`non-vacuity-ctl-only-wrappers`** *(routed next — the follow-on from `non-vacuity-operator-widening`)* — give `shellcheck.sh` and `runaway-killswitch-wired.sh` selftests a negative fixture that exercises their own wrapper, converting the two CTL-only equivalent mutants to KILLED (`28 · 0 · 3` of 31). Owner-routed as the next slice.
