...
- **E9 — environment-promotion vertical:** ✅ **DISSOLVED as-scoped 2026-07-02** — promotion is covered (`progressive-delivery.md`: promotion boundaries · smoke gates · canary · rollback; `preview-environments.md`), and config-as-code + least-privilege are 15-factor factors 5/8/15. Runtime IAM detective-scanning is correctly org/platform-owned. **BUT the brainstorm surfaced the real gap → the deployment-PLATFORM adoption axis** (the kit concretizes stack + harness but not the deploy-target; "org-owned" is labeled, not guided) — now the **active next epic** (a neutral obligations-contract + adopter-filled `DEPLOYMENT-ENVIRONMENT` manifest; the kit owns the questions, the adopter owns the answers).
