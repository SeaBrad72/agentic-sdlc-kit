...
**Current state:** `v3.92.0`. **Complete: E5 (observability/operate-loop) · E6 (AI-native eval depth) · E11 (AI-artifact lineage, resolved). Dissolved as covered: E7 (supply-chain) · E9 (env-promotion) — see below.** Skill spine = 11 content skills + the `using-skills` keystone. Roster = Orchestrator (Architect/Product/Ops hats) · Engineer×N · Reviewer · Security-reviewer · Kit-Steward (meta-control). **★ The two real remaining frontiers are adoption bridges, not core machinery: the deployment-PLATFORM axis (next) and the live-TEAM axis (T2). Then dogfood a real product → re-run the kit-evaluation → final cleanup.**
...
2. **BUILD · Deployment-PLATFORM adoption axis** *(✅ done, v3.95.0)* — the missing 3rd concretization axis (stack ✓, harness ✓, **deploy-target ✓**). A *neutral* obligations-contract recipe (`docs/adoption/DEPLOYMENT-ENVIRONMENT.md`) — **the kit owns the questions, the adopter owns the answers** (adopter answers live in RUNBOOK §4; AWS/K8s worked + an AWS/Azure/GCP topology table; runtime containment stays correctly org-owned). Both adoption bridges (vc-hosts + deploy-target) now indexed in CLAUDE.md.
...
- **E7 — best-practice & supply-chain adherence:** ✅ **DISSOLVED 2026-07-02** (scoping brainstorm). The foundation is comprehensive — 14 supply-chain/security gates (secret-scan · dep-scan · SBOM · image-SBOM · provenance · image-provenance · SAST · license · action-pinning · token-scope · …) + SOC2/ISO27001/NIST-SSDF/AI-RMF crosswalks + a 49-row audit-evidence checklist + SLSA Build L2. Every genuine absence is org/platform-owned, redundant, or build-ahead-without-a-consumer. **Banked:** `audit-evidence-export` (gated on a real audit/adopter — don't guess the format now).
- **E9 — environment-promotion vertical:** ✅ **DISSOLVED as-scoped 2026-07-02** — promotion is covered (`progressive-delivery.md`: promotion boundaries · smoke gates · canary · rollback; `preview-environments.md`), and config-as-code + least-privilege are 15-factor factors 5/8/15. Runtime IAM detective-scanning is correctly org/platform-owned. **BUT the brainstorm surfaced the real gap → the deployment-PLATFORM adoption axis** (the kit concretizes stack + harness but not the deploy-target; "org-owned" is labeled, not guided) — now **✅ shipped v3.95.0** (a neutral obligations-contract recipe `docs/adoption/DEPLOYMENT-ENVIRONMENT.md`; adopter answers live in RUNBOOK §4; the kit owns the questions, the adopter owns the answers).
...
- **`non-vacuity-operator-widening`** *(✅ SHIPPED, v3.85.0)* — region-model + `target_set` refinement drove the mutation-testing gate from `25 killed · 0 survived · 7 uncovered (of 32)` to **`26 killed · 0 survived · 5 uncovered (of 31) + 1 SKIPPED`** via three detection-only fixes: `first_marker` now requires a positional-arg test so an `if ! sh child --selftest` INVOCATION line no longer masks the real oracle (`non-vacuity.sh` now dogfoods itself, KILLED); `has_selftest_dispatch` gates `target_set` membership so a check whose `--selftest` is only a fixture payload (`agent-autonomy.sh`) is reported `SKIPPED` (visible, never silently dropped), not `UNCOVERED`; the `no-idiom` message now states the structural cause. No new mutation operators; further operator-breadth widening stays **banked, low priority**.
- **`non-vacuity-ctl-only-wrappers`** *(✅ SHIPPED, v3.86.0)* — `shellcheck.sh` and `runaway-killswitch-wired.sh` selftests each gained a fail-closed meta-fixture exercising their own wrapper (`run()`'s `return 1`; `fail()`'s `exit 1`), converting both CTL-only equivalent mutants to KILLED: `26 killed · 0 survived · 5 uncovered` -> **`28 killed · 0 survived · 3 uncovered (of 31) + 1 SKIPPED`**. **This completes the non-vacuity topic** — the remaining 3 UNCOVERED (`check-links` sibling teeth, `proportional-gate-wired` teeth-in-selftest, `mode-enforcement-blind` self-scan) are structural (no selftest oracle to prove vacuous), left honest; further operator-breadth widening stays **banked, low priority, not recommended**.
