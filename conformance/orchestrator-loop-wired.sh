...
if [ "${1:-}" = "--selftest" ]; then
  ...
  r24="$d/case24"; mkdir -p "$r24/agents" "$r24/scripts" "$r24/_gh_/workflows" "$r24/skills/design" "$r24/skills/plan" "$r24/skills/tdd" "$r24/skills/review" "$r24/skills/worktrees" "$r24/skills/verification" "$r24/skills/using-skills" "$r24/skills/debugging" "$r24/skills/evals" "$r24/skills/continuous-discovery"
  ...
  r25="$d/case25"; mkdir -p "$r25/agents" "$r25/scripts" "$r25/_gh_/workflows" "$r25/skills/design" "$r25/skills/plan" "$r25/skills/tdd" "$r25/skills/review" "$r25/skills/worktrees" "$r25/skills/verification" "$r25/skills/using-skills" "$r25/skills/debugging" "$r25/skills/evals" "$r25/skills/continuous-discovery"
  ...
  rm -rf "$d"
  if [ "$sf" -eq 0 ]; then echo "OK: orchestrator-loop-wired selftest (conflict-safe + design-skill + plan-skill + tdd-skill + review-skill + worktrees-skill + verification-skill + using-skills-keystone + debugging-skill + evals-skill + continuous-discovery-skill)"; exit 0
...
[ "$fail" -eq 0 ] && { echo "OK: orchestrator-loop wired (roster headings + A2 kill-switch + trusted-denial + conflict-safe + design-skill + plan-skill + tdd-skill + review-skill + worktrees-skill + verification-skill + using-skills-keystone + debugging-skill + evals-skill + continuous-discovery-skill + golden-path job)"; exit 0; }
